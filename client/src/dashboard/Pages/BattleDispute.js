...
const BattleDispute = () => {
  const { cancelBetData, loader } = useSelector((state) => state.admin);

  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(cancelBet());
  }, [dispatch, cancelBetData?.length]);
  ...
  const handleRefund = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(adminRefund(id)).then(() => {
          dispatch(admin_reset())
          window.location.reload();
        });
      } else if (result.isDismissed) {
        console.log("Cancelled");
        ...
        <div className="py-2">
          ...
          <div className="overflow-x-auto sm:w-[300px] md:w-[100%]">
            <table className=" leading-normal w-[100%]">
              ...
              <tbody>
                {Array.isArray(cancelBetData) &&
                  cancelBetData?.map((battle, index) => (
                    <tr key={battle.id}>
                      ...
                      <td className="px-1 py-5 border border-gray-200 bg-white text-[15px]">
                        <button
                          className="bg-[#4ad9ec] text-white hover:bg-red-900 flex items-center p-2 rounded-sm"
                          disabled={loader?true:false} 

                          onClick={() => handleRefund(battle._id)}
                        >
                          refund
                        </button>
                        <button
                          className="bg-red-500 text-white p-2 rounded mr-2 mt-3 text-sm"
