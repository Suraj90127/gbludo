...
const AllBets = () => {
  const { battleRecieveData, loader } = useSelector((state) => state.admin);
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(adminBattleRecieve());
  }, [dispatch, battleRecieveData?.length]);
        ...
        <div className="py-2">
          ...
          <div className="overflow-x-auto sm:w-[300px] md:w-[100%]">
            <table className=" leading-normal w-[100%]">
              ...
              <tbody>
                {Array.isArray(battleRecieveData) &&
                  battleRecieveData?.map((battle, index) => (
                    <tr key={battle.id}>
