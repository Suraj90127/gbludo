...
import {  adminSuccessKyc } from '../../store/reducer/adminReducer';

const ApprovedKyc = () => {
    const dispatch = useDispatch()
    const {userKycSuccess} = useSelector(state=>state.admin)
  ...
  const handleView = (id) => {
    dispatch(adminSuccessKyc(id)).then((res)=>{
      ...
      const filteredData = userKycSuccess?.filter(entry => 
        entry?.phone?.toString().includes(searchTerm)
      );
      useEffect(()=>{
        dispatch(adminSuccessKyc())
      },[])
  return (
    <div className='w-[100%]'>
       <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">KYC</h1>
      <h2 className="text-xl font-semibold mb-2">APPROVED KYC</h2>
      ...
      <div className="overflow-x-auto sm:w-[310px] md:w-[100%]">
        <table className="min-w-full border border-zinc-300">
          ...
          <tbody className='bg-white'>
            {Array.isArray(userKycSuccess) && filteredData.map((entry, index) => (
              <tr key={entry.id}>
